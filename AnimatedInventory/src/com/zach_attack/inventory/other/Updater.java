...
public class Updater {
    private static final String USER_AGENT = "AnimatedInventory Auto-Updater";
    ...
    public Updater(Plugin plugin, File file, UpdateType updateType, boolean logger) {
        this.plugin = plugin;
        this.updateFolder = plugin.getServer().getUpdateFolderFile();
        this.id = 59785; // Resource ID
        this.file = file;
    ...
    private void checkUpdate() {
        try {
            ...
            if (jsonArray.size() == 10 && !emptyPage) {
                ...
                if (logger) {
                    plugin.getLogger().info("Checking Spigot for possible Updates...");
                }
            ...
            if(AnimatedInventoryAPI.debugMode) {
            	plugin.getLogger().warning("Error trying to check for an update:");
            	e.printStackTrace();
            }
        }
    }
    ...
    private void download() {
        ...
        try {
            URL url = new URL(downloadLink); in = new BufferedInputStream(url.openStream());
